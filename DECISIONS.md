...
| D21b | **Q12: a batch takes its real number when it goes live, not before** | A Draft carries a temporary internal reference (`draft-xxxxxx`), never printed and never public. The global, sequential, zero-padded number is stamped at Draft to Open, inside the counter transaction, and the document moves to `batches/{nnn}`. An abandoned draft burns no number, so printed numbers run unbroken. Note: the real number cannot wait until cooking, because brief §8.4 puts the open batch on `/batch/<nnn>` while customers are booking |
